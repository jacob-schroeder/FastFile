...
public partial class XFileReader
{
    ...
    {
        if (!attr.OffsetIsAliasCell ||
            pointerObj is not FastFile.ModelsOLD.Zone.Pointer pointer ||
        ...
        WithStreamBlock(address.Block, () =>
        {
            ...
            if (attr.ElementTarget != XPointerTarget.None)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    {
                        throw new InvalidDataException(
                            $"Failed to resolve {targetType.Name} pointer array element {i} ({DescribePointer((FastFile.ModelsOLD.Zone.Pointer)pointer)}).",
                            ex);
                    }
                }
            }
    ...
    private static string DescribePointer(FastFile.ModelsOLD.Zone.Pointer pointer)
    {
    ...
    {
        if (value is FastFile.ModelsOLD.Zone.Pointer pointer)
            return DescribePointer(pointer);

        if (value is Array array)
        {
            var first = array.Cast<object?>().FirstOrDefault(item => item is FastFile.ModelsOLD.Zone.Pointer);
            return first is FastFile.ModelsOLD.Zone.Pointer firstPointer
