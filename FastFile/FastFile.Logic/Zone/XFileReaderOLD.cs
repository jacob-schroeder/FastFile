...
public sealed class XFileReaderOLD
{
	...
	public XFileReaderOLD(byte[] buffer, Action<int, int>? assetReadProgress = null)
	{
	...
	public XAssetListOLD Load_XAssetList()
	{
		EnsureHeaderParsed();
		...
		try
		{
			...
			var assetList = new XAssetListOLD
			{
		...
		finally
		{
			...
			File.WriteAllBytes("/Users/jacob/Downloads/streams/temp.block", _streamBlocks[(int)XFILE_BLOCK.TEMP]);
			File.WriteAllBytes("/Users/jacob/Downloads/streams/large.block", _streamBlocks[(int)XFILE_BLOCK.LARGE]);
		}
	}

	private void LoadScriptStringList(XAssetListOLD assetListOld)
	{
		var pointer = assetListOld.ScriptStringsPtr;
		var count = assetListOld.ScriptStringCount;
	...
	private void LoadXAssetArray(XAssetListOLD assetListOld)
	{
		var pointer = assetListOld.AssetsPtr;
		var count = assetListOld.AssetCount;
